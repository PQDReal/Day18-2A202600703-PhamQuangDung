...
def generate_hypothesis_questions(text: str, n_questions: int = 3) -> list[str]:
    ...
    for sentence in sentences[:n_questions]:
        if re.search(r"\d+", sentence):
            questions.append(f"{sentence.rstrip('.')} là bao nhiêu?")
        else:
...
def extract_metadata(text: str) -> dict:
    ...
    lower = text.lower()
    if any(word in lower for word in ["mật khẩu", "vpn", "mfa", "malware"]):
        category = "it"
    elif any(word in lower for word in ["lương", "phụ cấp", "tạm ứng", "chi phí"]):
        category = "finance"
    elif any(word in lower for word in ["nghỉ", "nhân viên", "thử việc", "bảo hiểm"]):
        category = "hr"
    else:
        category = "policy"
    topic = summarize_chunk(text)[:80]
    entities = sorted(set(re.findall(r"\b[A-ZĐ][\wÀ-ỹ-]{2,}\b", text)))[:8]
